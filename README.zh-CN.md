...
| **自动去重** | 按笔记 ID 对照持久化索引，重复导入同一篇会跳过而不是生成第二份。 |
...
| **作者小红书号** | 可选抓取作者主页，记录**小红书号**、简介、粉丝数与主页链接。按作者缓存。 |
...
| `{{author}}` | 作者昵称 |
| `{{authorId}}` | 作者 ID |
| `{{authorRedId}}` | **作者小红书号** —— 需额外一次主页请求 |
| `{{authorDesc}}` | 作者简介 |
| `{{authorUrl}}` | 作者主页链接 |
| `{{authorFans}}` | 作者粉丝数 |
| `{{authorIpLocation}}` | 作者 IP 属地 |
| `{{likedCount}}` | 点赞数 |
| `{{collectedCount}}` | 收藏数 |
| `{{commentCount}}` | 评论数 |
| `{{shareCount}}` | 分享数 |
...
| 开启「抓取作者小红书号」 | 向作者在 `www.xiaohongshu.com` 的公开主页发起 HTTP GET | 读取作者的**小红书号**、简介与粉丝数 —— 这些信息不在笔记页下发。按作者缓存，同一作者每次会话最多请求一次 |
